...
- **朋友端**：看菜单 / 选日子（午宴·晚宴）/ 点菜下单（可选口味）/ 我的点单
- **共享点单**：同一（日期 + 午/晚饭）合并为一个共享订单，朋友之间互相可见、可互相改删数量；主人确认后锁定
- **主人端**（管理后台 `/admin`）：看板 / 菜品管理 / 订单管理 / 屏蔽不可约时间
- **通知**：每次有人加菜 / 改单 / 删菜都发邮件（Resend，含口味选择与备料克重汇总）+ 后台待办列表
- **每道菜可配置**：口味选项组（如 辣度：微辣/中辣/重辣，必选或选填）与每份备料克重（如 牛腩 250g）；邮件里自动按「每份克重 × 份数」汇总采购清单（同名配料自动合并克重）
...
| `NEXT_PUBLIC_SITE_URL` | 可选。你的正式网址（如 `https://xx.vercel.app`），用于邮件里的「去管理后台」链接。不填会自动用部署网址（Vercel 的 `VERCEL_URL` 或下单请求的地址），一般无需设置 |
...
| --- | --- |
| 看菜单 / 点菜 / 预约 | 首页（公开链接） |
| 管理菜单（加菜、下架、改价、口味选项、备料克重） | `/admin/menu` |
| 屏蔽某天 / 某时段（午/晚饭） | `/admin/schedule` |
| 确认 / 完成 / 取消订单 | `/admin/orders` |
| 朋友点单 / 互相修改 | 首页选好（日期+午/晚饭）后，大家往同一个共享订单加菜，可互相改数量/删除；主人点「确认接单」后锁定 |
